...
namespace AutomationApp
{
    public partial class Form1 : Form
    {
        ...
        private void lgnbtn_Click(object sender, EventArgs e)
        {
            ...
            check.Parameters.AddWithValue("@password",passtxtbox.Text);
            check.CommandText = "SELECT COUNT(*) FROM RST_DB.login where user_email= @email and pass= @password";
            connect.Open();
            int results = (int)(long)check.ExecuteScalar();
            if (results > 0)
            {
                this.Close();
                Process.Start(@"C:\Users\owens\Desktop\Automation\WalmartAutomation\bin\Debug\net5.0\WalmartAutomation.exe");
            }
            else
            {
